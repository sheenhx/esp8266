   ...
   tmr.alarm(2, 1000, 0, function() --delay the setup incase of memory shortage
      setup_mpr121()
    end)
    ...
    tmr.alarm(0, relayrfq or 500, 1, function()  --check the register using the received parameter
      ...
      local pbit2= string.byte(reg2)
    if pbit1 ~= 0 or pbit2 ~= 0 then
      m:publish("/v1/"..USER.."/data", ' [{"meaning":"btnreg1","value": '..pbit1..'},{"meaning":"btnreg2","value": '..pbit2..'}]',0,0)
    end
   end)
